...
| Phase 7 | Multi-platform ripple: `snapshot_all_holdings` daily job uses `PortfolioValueHistory` uniqueness `(snapshot_date, ticker)` only → a ticker held on 2 platforms silently records ONE platform's snapshot per day (value-history chart undercounts). Fix: key snapshots per `(snapshot_date, ticker, platform_id)`. | Open | rb2 (2026-07-11) |
