...
| 2026-09-04 | 260904-pvh-collapsible-sidebar | Collapsible icons-only left sidebar in `ui/app/components/Nav.tsx` — chevron toggle shrinks the rail 236px→68px, hiding wordmark/Menu/labels/footer, keeping icons; collapsed state persists to `localStorage["monai.sidebarCollapsed"]` (hydrated in effect, SSR-safe); a11y aria-labels on icon-only links + toggle. tsc clean. Commit `c6000e5`. Needs frontend rebuild to view live. |
| 2026-09-04 | portfolio-chart-match-trend | Restyled `ui/app/investments/ValueHistoryChart.tsx` to the cashflow TrendChart's visual language — styles.ts tokens (was hardcoded hex), dropped CartesianGrid + YAxis, borderless monotone line strokeWidth 2.4, matching tooltip, 170px height. Kept its card + range/view controls. Commit `eff5b5d`. |
...
| 2026-09-04 | networth-line-trend-chart | Net-worth line on the cashflow trend chart. Backend `net_worth_trend()` + GET /cashflow/networth-history = monthly liquid (all liquid-account tx to month-end) + investment (latest portfolio_value_history snapshot ≤ month-end); null where no snapshot exists (no fabricated jump — snapshots began 2026-07-11). +test. Frontend plots it as a 3rd ink line on a separate hidden Y-axis (net worth ≈100× income/expense), connectNulls=false. Commit `67df1f3`. Verified live: Jul/Aug/Sep real, Apr–Jun null, current month matches /net-worth. |
